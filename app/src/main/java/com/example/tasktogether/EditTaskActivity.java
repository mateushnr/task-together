...
public class EditTaskActivity extends AppCompatActivity implements RemoveMemberListener {
    ...
    ArrayList<UserTask> currentResponsiblesForTask;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView txvTaskToEditName = findViewById(R.id.txvTaskToEditName);
        ...
        Intent intentEditTask = getIntent();

        ArrayList<UserGroup> membersGroupList;

        selectedGroup = null;

        if(intentEditTask.hasExtra("idTaskToEdit")){
            int idSelectedTask = intentEditTask.getIntExtra("idTaskToEdit", 0);
            selectedTask = taskDao.searchById(idSelectedTask);

            if(loggedIn){
                currentResponsiblesForTask = userTaskDao.searchAllByTask(idSelectedTask);

                for(UserTask responsible: currentResponsiblesForTask){
                    User user = userDao.searchById(responsible.getIdUser());
                    if(responsible.getIdUser() == loggedUser.getIdUser()){
                        user.setName("Você");
                    }

                    responsiblesForTask.add(user);
                    responsiblesUsersId.add(user.getIdUser());
                }
            }


            membersGroupList = userGroupDao.searchAllByGroupWhereIsMember(selectedTask.getIdGroup());

            for(UserGroup memberGroup: membersGroupList){
                userIdMembers.add(memberGroup.getIdUser());
            }

            txvTaskToEditName.setText(selectedTask.getName());

            edtTaskName.setText(selectedTask.getName());

            edtTaskDescription.setText(selectedTask.getDescription());

            edtTaskLimitDate = findViewById(R.id.edtTaskLimitDate);
            edtTaskLimitDate.setText(selectedTask.getLimitDate());
        }
        ...
        btnCreateTask.setOnClickListener(v -> {
            ...
            if(isValid){
                editTask(edtTaskName.getText().toString(), edtTaskDescription.getText().toString(), edtTaskLimitDate.getText().toString(), loggedUserEmail);
            }
        });
    }
    ...
    public void editTask(String task_name, String task_description, String task_limit_date, String ownerEmail){
        TaskDAO taskDAO = new TaskDAO(this);
        UserTaskDAO userTaskDao = new UserTaskDAO(this);

        selectedTask.setName(task_name);
        selectedTask.setDescription(task_description);
        selectedTask.setLimitDate(task_limit_date);
        ...
        if(!Objects.equals(selectedTask.getStatus(), "Concluida") ){
            ...
            try {
                ...
                if (limitDate.before(currentDate)) {
                    selectedTask.setStatus("Atrasada");
                } else {
                    selectedTask.setStatus("Pendente");
                }
        ...
        if(ownerEmail != null){
            for(UserTask currentResponsible: currentResponsiblesForTask){
                boolean wasDeleted = true;

                for(int responsibleId: responsiblesUsersId){
                    if(currentResponsible.getIdUser() == responsibleId){
                        wasDeleted = false;
                    }
                }

                if(wasDeleted){
                    userTaskDao.delete(currentResponsible.getIdUserTask());
                }
            }
            ...
            for(int responsibleId: responsiblesUsersId){
                ...
                UserTask responsibleToAdd = new UserTask();
                ...
                if(wasNotAdded){
                    responsibleToAdd.setIdTask(selectedTask.getIdTask());
                    responsibleToAdd.setIdUser(responsibleId);

                    userTaskDao.insert(responsibleToAdd);
                }
            }
        }

        Intent taskEditedIntent = new Intent(this, TaskActivity.class);
        taskEditedIntent.putExtra("selectedTask", selectedTask.getIdTask());
        startActivity(taskEditedIntent);

        Toast.makeText(this, "Tarefa editada!",
